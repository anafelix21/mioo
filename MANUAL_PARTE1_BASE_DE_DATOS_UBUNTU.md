# 📘 MANUAL PARTE 1: CONFIGURAR INSTANCIA DE BASE DE DATOS (UBUNTU)
## Carnicería Pochito - MySQL en EC2 Ubuntu

---

## 🎯 OBJETIVO

Configurar una instancia EC2 en AWS con Ubuntu y MySQL para alojar la base de datos de la aplicación.
...
ssh -i "pochito-key.pem" ubuntu@XX.XX.XX.XX
```
...
## 🗄️ PASO 3: INSTALAR MYSQL EN UBUNTU
...
scp -i "C:\Users\TU_USUARIO\Downloads\pochito-key.pem" "C:\ruta\a\database.sql" ubuntu@XX.XX.XX.XX:/home/ubuntu/
```

```bash
# Linux/Mac
scp -i ~/Downloads/pochito-key.pem /ruta/a/database.sql ubuntu@XX.XX.XX.XX:/home/ubuntu/
```
...
ls -lh /home/ubuntu/database.sql

# Importar la base de datos
mysql -u pochito_user -p pochitoweb < /home/ubuntu/database.sql
...
👤 USUARIOS ADICIONALES (si los creaste en database.sql):
   - wilfredo / 12345678
   - jimena / jimena123456
...
✅ Instancia EC2 con Ubuntu creada  
...
- Usuario correcto para Ubuntu: `ubuntu` (no `admin`)
...
# Intentar con sudo
sudo mysql pochitoweb < /home/ubuntu/database.sql
```

### Error "Access denied for user"
```bash
# Verificar que el usuario existe
sudo mysql -u root -e "SELECT User, Host FROM mysql.user WHERE User = 'pochito_user';"

# Recrear usuario si es necesario
sudo mysql -u root
DROP USER IF EXISTS 'pochito_user'@'%';
CREATE USER 'pochito_user'@'%' IDENTIFIED BY 'Pochito2025!Secure';
GRANT ALL PRIVILEGES ON pochitoweb.* TO 'pochito_user'@'%';
FLUSH PRIVILEGES;
EXIT;
