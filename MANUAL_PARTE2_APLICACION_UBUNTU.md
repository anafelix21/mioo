# 📘 MANUAL PARTE 2: CONFIGURAR INSTANCIA DE APLICACIÓN WEB (UBUNTU)
## Carnicería Pochito - Flask + Nginx en EC2 Ubuntu

---

## 🎯 OBJETIVO

Configurar una instancia EC2 en AWS con Ubuntu, instalar la aplicación web Flask, conectarla a la base de datos y ponerla en producción.
...
🖥️ Application and OS Images:
   - Quick Start: Ubuntu
   - Seleccionar: Ubuntu Server 22.04 LTS (64-bit x86)
...
```powershell
# PowerShell (Windows)
ssh -i "C:\ruta\a\pochito-key.pem" ubuntu@XX.XX.XX.XX
```

```bash
# Linux/Mac
ssh -i /ruta/a/pochito-key.pem ubuntu@XX.XX.XX.XX
```
...
# Instalar herramientas de desarrollo para MySQL
...
# (venv) ubuntu@ip-172-31-XX-XX:~/mioo$
```
...
**Busca estas líneas (alrededor de la línea 27-33):**
...
**Cambiar `172.31.XX.XX` por la IP PRIVADA real de tu instancia de base de datos.**

Ejemplo:
```python
app.config["MYSQL_HOST"] = "172.31.45.123"  # ⬅️ Tu IP privada de BD
```
...
User=ubuntu
Group=www-data
WorkingDirectory=/home/ubuntu/mioo
Environment="PATH=/home/ubuntu/mioo/venv/bin"
ExecStart=/home/ubuntu/mioo/venv/bin/gunicorn --workers 3 --bind 0.0.0.0:5000 app:app
...
server {
    ...
    location /favicon.ico {
        alias /home/ubuntu/mioo/static/image/favicon.ico;
        access_log off;
    }
...
sudo chmod -R 755 /home/ubuntu/mioo/static

# Crear directorio de imágenes si no existe
mkdir -p /home/ubuntu/mioo/static/image

# Dar permisos de escritura para subir imágenes
sudo chown -R ubuntu:www-data /home/ubuntu/mioo/static/image
sudo chmod -R 775 /home/ubuntu/mioo/static/image
```
...
- [x] Instancia de Base de Datos (Ubuntu) configurada y corriendo
- [x] MySQL instalado y base de datos importada
- [x] Instancia de Aplicación (Ubuntu) creada y configurada
...
```bash
# En la instancia de BD, verificar que MySQL está corriendo
sudo systemctl status mysql

# Verificar Security Groups en AWS Console
# Asegurarse de que el puerto 3306 permite conexiones desde Pochito-App-SG
```
...
```bash
# Verificar permisos
ls -la /home/ubuntu/mioo/static/image

# Corregir permisos
sudo chown -R ubuntu:www-data /home/ubuntu/mioo/static
sudo chmod -R 755 /home/ubuntu/mioo/static
```
...
```bash
# Desde la instancia de App, probar conexión a MySQL
mysql -h [IP-PRIVADA-BD] -u pochito_user -p
