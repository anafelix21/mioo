/* ============================================
   CATEGORÍA: ENCENDIDO
   Carga dinámica de combustibles y encendido
   ============================================ */

// Cargar productos de categoría 8 (Combustible y Encendido)
async function cargarProductos() {
    try {
        const response = await fetch('/api/productos/categoria/8');
        const data = await response.json();

        if (data.success && data.productos.length > 0) {
            const html = data.productos.map(prod => `
                <div class="encendido-card bg-white rounded-xl shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl">
